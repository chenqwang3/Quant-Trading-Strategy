...
for(i in 1:nrow(all.budget)){
  #current week: week i 
  all.share[i,1:length(stock.list)]<-as.numeric(all.budget[i,1:length(stock.list)])/(as.numeric(all.price[i,2:length(stock.list)+1])*1)   
}
...
(8638*92.84+5495*36.34+53962*48.71)/0.9-4265*4.66858+4265*7.17
...
plot(sr$Date,sr$sr,type='l',xlab='time',ylab='Sharpe Ratio',
     main='Line plot of Sharpe Ratio over time (Reblancing, TC=0)')
hist(sr$sr,main='Histogram of Sharpe Ratio (Rebalancing, TC=0)',
     xlab='Sharpe Ratio')
...
plot(h, xlab='Monthly excess return',
     ylab='Probability',main='VaR: Distribution of monthly excess return (Rebalancing, TC=0)')