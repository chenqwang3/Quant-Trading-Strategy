...
tc<-0 #set the transaction cost for all trades on eight assets
...
plot(sr$Date,sr$sr,type='l',xlab='time',ylab='Sharpe Ratio',
     main='Line plot of Sharpe Ratio over time (Moving average, TC=0%)')
hist(sr$sr,main='Histogram of Sharpe Ratio (Moving average, TC=0%)',
     xlab='Sharpe Ratio')
...
plot(h, xlab='Monthly excess return',
     ylab='Probability',main='VaR: Distribution of monthly excess return (Moving Average, TC=0%)')
...
total.port.value #portfolio value $8,767,226
total.port.rtn<-total.port.value/1000000-1 #portfolio return 7.767226
ben.rtn
total.port.rtn
