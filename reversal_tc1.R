...
tc<-0.01 #transaction cost for all stocks
...
monthly.rf<-monthly.rf[12:(nrow(monthly.rf)-1),] #Dec 2017 to Nov 2022
...
plot(sr$Date,sr$sr,type='l',xlab='time',ylab='Sharpe Ratio',
     main='Line plot of Sharpe Ratio over time (Reversal, TC=1%)')
hist(sr$sr,main='Histogram of Sharpe Ratio (Reversal, TC=1%)',
     xlab='Sharpe Ratio')

#total value of portfolio at the end of trading period
perc.monthly.pos[nrow(perc.monthly.pos),'port.value'] #$1335543

##Average of monthly excess return
mean(sr$rtn) #0.01662589
...
plot(h, xlab='Monthly excess return',
     ylab='Probability',main='VaR: Distribution of monthly excess return (Reversal, TC=1%)')
...
total.port.rtn<-perc.monthly.pos[nrow(perc.monthly.pos),'port.value']/1000000-1 #portfolio return 0.3355427
ben.rtn
total.port.rtn #cumulative return 33.55%
