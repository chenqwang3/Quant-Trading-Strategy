...
for(i in 1:nrow(all.budget)){
  #current week: week i 
  all.share[i,1:length(stock.list)]<-as.numeric(all.budget[i,1:length(stock.list)])/(as.numeric(all.price[i,2:length(stock.list)+1])*1.01)   
}
...
(8553*92.84+5440*36.34+53428*48.71)/0.9-4222*4.66858+4222*7.17
...
plot(sr$Date,sr$sr,type='l',xlab='time',ylab='Sharpe Ratio',
     main='Line plot of Sharpe Ratio over time (Reblancing, TC=1%)')
hist(sr$sr,main='Histogram of Sharpe Ratio (Rebalancing, TC=1%)',
     xlab='Sharpe Ratio')
