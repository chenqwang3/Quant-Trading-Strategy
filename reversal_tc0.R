...
tc<-0 #transaction cost for all stocks
...
plot(sr$Date,sr$sr,type='l',xlab='time',ylab='Sharpe Ratio',
     main='Line plot of Sharpe Ratio over time (Reversal, TC=0%)')
hist(sr$sr,main='Histogram of Sharpe Ratio (Reversal, TC=0%)',
     xlab='Sharpe Ratio')

#total value of portfolio at the end of trading period
perc.monthly.pos[nrow(perc.monthly.pos),'port.value'] #3177830

##Average of monthly excess return
mean(sr$rtn) # 0.01192203
...
plot(h, xlab='Monthly excess return',
     ylab='Probability',main='VaR: Distribution of monthly excess return (Reversal, TC=0%)')
...
ben.rtn #0.3075244
total.port.rtn #cumulative return 2.17783
